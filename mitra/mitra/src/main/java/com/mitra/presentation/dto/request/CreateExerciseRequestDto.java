...
public record CreateExerciseRequestDto(
        ...
        @NotBlank(message = "Muscle group is required")
                ...
                String muscleGroup,
        @NotNull(message = "MET factor is required") @DecimalMin(value = "0.1", message = "MET factor must be positive")
                @Schema(description = "Metabolic Equivalent of Task (MET) factor", example = "6.0")
                BigDecimal metFactor,
        @NotNull(message = "Tracking type is required") @Schema(
                        description =
                                "How this exercise is tracked (WEIGHT_REPS, REPS_ONLY, TIME_ONLY)",
                        example = "WEIGHT_REPS")
