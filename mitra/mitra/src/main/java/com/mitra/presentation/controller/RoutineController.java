...
public class RoutineController {
    ...
    @Operation(
            ...
            description = "Creates a new empty workout routine for the authenticated user")
    ...
    @Operation(
            ...
            description = "Retrieves all workout routines belonging to the authenticated user")
    ...
    public ResponseEntity<Page<RoutineResponseDto>> getMyRoutines(
        ...
        Page<RoutineResponseDto> routines =
                getWorkoutRoutinesUseCase.execute(currentUser.getId(), pageable);
        return ResponseEntity.ok(routines);
    }

    @Operation(
            summary = "Add an exercise to a routine",
            description = "Maps an exercise to a given routine with target sets and reps")
    @ApiResponse(responseCode = "200", description = "Exercise added to routine successfully")
    @ApiResponse(
            responseCode = "403",
            description = "Routine does not belong to the authenticated user")
    ...
    public ResponseEntity<RoutineExerciseResponseDto> addExerciseToRoutine(
            ...
            @AuthenticationPrincipal User currentUser) {
        RoutineExerciseResponseDto response =
                addRoutineExerciseUseCase.execute(currentUser.getId(), routineId, request);
