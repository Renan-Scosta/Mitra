...
public class GetUserVolumeSummaryUseCaseImpl implements GetUserVolumeSummaryUseCase {
    ...
    @Override
    public List<VolumeSummaryResponseDto> execute(
            Long userId, LocalDateTime startDate, LocalDateTime endDate) {
        log.debug(
                "Calculating volume summary userId={} range={} to {}", userId, startDate, endDate);
        List<WorkoutSession> sessionsInRange =
                sessionRepositoryPort.findByUserId(userId).stream()
                        .filter(
                                s ->
                                        s.getStartTime() != null
                                                && !s.getStartTime().isBefore(startDate)
                                                && !s.getStartTime().isAfter(endDate))
                        .toList();

        Map<String, Double> volumeByMuscleGroup =
                sessionsInRange.stream()
                                        ...
                                        set ->
                                                set.getExercise() != null
                                                                && set.getExercise()
                                                                                .getMuscleGroup()
                                                                        != null
                                                        ? set.getExercise().getMuscleGroup()
                                                        : "UNKNOWN",
                                        Collectors.summingDouble(
                                                set ->
                                                        set.getWeightKg().doubleValue()
                                                                * set.getReps())));
