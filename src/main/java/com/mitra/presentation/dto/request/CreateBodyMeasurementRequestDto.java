...
public record CreateBodyMeasurementRequestDto(
        @NotNull(message = "Weight is required") @DecimalMin(value = "20.0", message = "Weight must be at least 20 kg")
                @Schema(description = "Body weight in kg", example = "82.3")
                BigDecimal weightKg,
        @Schema(description = "Body fat percentage (optional)", example = "18.5")
                BigDecimal bodyFatPercentage,
        @NotNull(message = "Record date is required") @Schema(description = "Date of the measurement", example = "2026-04-16")
