...
public record CreateUserRequestDto(
        ...
        @NotNull(message = "Birth date is required") @Past(message = "Birth date must be in the past")
                ...
                LocalDate birthDate,
        @NotNull(message = "Gender is required") @Schema(description = "Biological gender (MALE/FEMALE)", example = "MALE")
                Gender gender,
        @Min(value = 50, message = "Height must be at least 50 cm")
                @Max(value = 300, message = "Height must be at most 300 cm")
                @Schema(description = "Height in centimeters", example = "180")
                int heightCm,
        @NotNull(message = "Initial weight is required") @DecimalMin(value = "20.0", message = "Weight must be at least 20 kg")
