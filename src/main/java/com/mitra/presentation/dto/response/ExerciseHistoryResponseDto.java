...
public record ExerciseHistoryResponseDto(
        ...
        @Schema(description = "Exercise name", example = "Deadlift") String exerciseName,
        @Schema(
                        description =
                                "List of sessions where this exercise was performed, ordered"
    ...
    public record SessionHistoryDto(
            @Schema(description = "Session ID", example = "100") Long sessionId,
            @Schema(description = "Date and time of the session", example = "2026-04-10T16:20:00")
                    LocalDateTime date,
            @Schema(description = "Sets performed in this session") List<SetHistoryDto> sets) {}
