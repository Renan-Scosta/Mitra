...
public class GetUserSessionsUseCaseImpl implements GetUserSessionsUseCase {
    ...
    public Page<WorkoutSessionResponseDto> execute(
        ...
        return workoutSessionRepositoryPort
                .findByUserIdAndDateRange(userId, startDateTime, endDateTime, pageable)
                .map(this::toDto);
    }
    ...
    private WorkoutSessionResponseDto toDto(WorkoutSession session) {
        ...
        if (session.getSetRecords() != null) {
            setDtos =
                    session.getSetRecords().stream()
                            .map(
                                    r ->
                                            new SetRecordResponseDto(
                                                    r.getId(),
                                                    r.getExercise().getId(),
                                                    r.getWeightKg(),
                                                    r.getReps(),
