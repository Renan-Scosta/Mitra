...
class CreateBodyMeasurementUseCaseImplTest {
    ...
    void shouldCreateMeasurementForUser() {
        CreateBodyMeasurementRequestDto request =
                new CreateBodyMeasurementRequestDto(
        ...
        assertEquals(10L, id);
        verify(bodyMeasurementRepositoryPort)
                .save(
                        argThat(
                                bm ->
                                        bm.getUserId().equals(1L)
                                                && bm.getWeightKg()
                                                                .compareTo(new BigDecimal("82.5"))
                                                        == 0
                                                && bm.getBodyFatPercentage()
                                                                .compareTo(new BigDecimal("19.0"))
                                                        == 0));
    }
    ...
    void shouldCreateMeasurementWithoutBodyFat() {
        ...
        assertEquals(11L, id);
        verify(bodyMeasurementRepositoryPort)
                .save(argThat(bm -> bm.getBodyFatPercentage() == null));
    }
}
