...
class BodyMeasurementControllerTest {
    ...
    @MockitoBean private CreateBodyMeasurementUseCase createBodyMeasurementUseCase;

    @MockitoBean private GetBodyMeasurementsUseCase getBodyMeasurementsUseCase;
    ...
    void shouldReturnMeasurementHistory() throws Exception {
        List<BodyMeasurementResponseDto> measurements =
                List.of(
                        new BodyMeasurementResponseDto(
                                1L,
                                new BigDecimal("82.5"),
                                new BigDecimal("19.0"),
                                new BigDecimal("66.83"),
                                new BigDecimal("15.68"),
                                LocalDate.of(2026, 4, 16)));

        when(getBodyMeasurementsUseCase.execute(eq(1L), any()))
                .thenReturn(new org.springframework.data.domain.PageImpl<>(measurements));
